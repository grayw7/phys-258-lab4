...
def calculate_phase_points(omega_drives_rads, all_times, all_amps, all_drives):
    ...
    for trial_num, (omega_drive_rads, time, amp, drive) in enumerate(zip(
        ...
        #FIXED BY NORMILZING PHASE TO -pi -> pi
        phase_diff = ((amp_phase - drive_phase) + np.pi) % (2 * np.pi) - np.pi
...
# our driving frequency
omega_drives_rads = np.array([0.36,0.46,0.56,0.66,0.76,0.86,0.96,1.06,1.16,1.26,1.36])
